...
def explain_neighbourhood(row: pd.Series) -> str:
    ...
    return f"{_safe(row['neighbourhood'])} stands out because " + ", ".join(parts) + "."
...
def explain_recommendation(row: pd.Series, preferences: dict[str, float]) -> str:
    ...
    if abs(adjustment) >= 0.015:
        direction = "boosted" if adjustment > 0 else "tempered"
        adjustment_text = f" The match was slightly {direction} by compatibility checks."

    return (
        f"{_safe(row['neighbourhood'])} ranks <strong>#{int(row.get('rank', 0))}</strong> "
        f"with a match score of <strong>{score:.0%}</strong>. "
        f"For your query, the strongest matching signals are: {metric_text}."
        f"{adjustment_text}"
    )
